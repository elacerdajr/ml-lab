...
run_experiment.py
-----------------
Thin entry point (repo convention) for the CatBoost-encoder experiment.

Prepends this folder to ``sys.path`` (so ``import cbenc`` works) and the repo
root (so ``import ml_elements`` works), then delegates to ``cbenc.main.main``.
...
sys.path.insert(0, str(SCRIPT_DIR))   # import cbenc
sys.path.insert(0, str(REPO_ROOT))    # import ml_elements

from cbenc.main import main  # noqa: E402
