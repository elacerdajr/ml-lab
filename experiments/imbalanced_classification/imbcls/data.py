...
Key rule
--------
``pi`` — the prior used by every prior mechanism — is the **true training base
rate** ``y_train_full.mean() ≈ 0.001``, never the 10% undersampled rate. Models
train on ``train_under`` but are evaluated on ``val_full`` / ``test_full`` which
preserve the real base rate, so they are judged under the deployment distribution.
...
sample_for_umap      Representative sample (all positives + sampled negatives).
...
def generate_full(cfg: Config) -> pd.DataFrame:
    ...
    pandas.DataFrame
        ...
        carried through every split (needed by the deterministic noise score).
    """
    d = cfg.data
...
def sample_for_umap(df: pd.DataFrame, n_target: int, seed: int) -> pd.DataFrame:
    """
    Representative subsample for UMAP: keep all positives, then sample negatives to
    reach ``n_target`` rows (or return all rows if the frame is already smaller).
    """
    if len(df) <= n_target:
    ...
    pos = df[df["y"] == 1]
    neg = df[df["y"] == 0]
    n_neg = max(0, n_target - len(pos))
    n_neg = min(n_neg, len(neg))
    neg_idx = rng.choice(len(neg), size=n_neg, replace=False)
    out = pd.concat([pos, neg.iloc[neg_idx]], ignore_index=True)
    return out.sample(frac=1.0, random_state=seed + 1).reset_index(drop=True)
