"""
run_study_imbalance.py
----------------------
Run Study 1 only: effect of class imbalance (p_pos sweep).

Imports all shared logic from run_experiment.py so there is no duplication.

Usage
-----
    python run_study_imbalance.py
    python run_study_imbalance.py --config config.yaml \\
                                  --study-config study_imbalance.yaml
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Study 1: class imbalance sweep.")
    p.add_argument("--config",       type=Path, default=SCRIPT_DIR / "config.yaml")
    p.add_argument("--study-config", type=Path, default=SCRIPT_DIR / "study_imbalance.yaml")
    return p.parse_args()
...
def main() -> None:
    ...
    log.info("=== Study 1: %s (%d conditions) ===", s["label"], len(conditions))
    ...
    plot_score_space(
        ...
        title=(
            "AUC vs AP Score Space — Class Imbalance\n"
