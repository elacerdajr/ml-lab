...
Research question
~~~~~~~~~~~~~~~~~
When the feature set is wide but heterogeneous — 100 features whose
information levels are drawn from Beta(1,9) (mean 0.10, strongly
right‑skewed) — how does training sample size drive the gap between
a high‑capacity booster (CatBoost) and interpretable rule/tree models
(FIGS, GreedyRuleList, plain DecisionTree)? Logistic regression serves
as the near‑Bayes reference (the DGP is linear in the features).

This replicates ``weak_features_sample_size`` but replaces the
constant ``info_j = 0.10`` with ``info_j ~ Beta(1, 9)``, keeping the
same average feature strength while introducing realistic heterogeneity.
...
that isolates per‑setup failures and records NaN.

Usage
~~~~~
    python run_experiment.py
    # or
    make exp-weak-features-beta
"""
...
def build_dgp(cfg: dict) -> tuple[GaussianBinaryDGP, list[str], dict[str, float]]:
    n_feat = cfg["dgp"]["n_features"]
    features = [f"x{i:02d}" for i in range(n_feat)]
    dist = cfg["dgp"]["info_dist"]
    rng = np.random.default_rng(dist["seed"])
    info = {f: float(rng.beta(dist["a"], dist["b"])) for f in features}
    dgp = GaussianBinaryDGP(
        p_pos=cfg["dgp"]["p_pos"],
        info=info,
        sigma=cfg["dgp"]["sigma"],
    )
    return dgp, features, info
...
def run() -> None:
    cfg = load_config()
    dgp, features, info_dict = build_dgp(cfg)
    ...
    # ── Save per‑feature info ────────────────────────────────────────────
    info_df = pd.DataFrame(
        {"feature": list(info_dict.keys()), "info": list(info_dict.values())}
    ).sort_values("info", ascending=False)
    info_df.to_csv(OUT_DIR / "feature_info.csv", index=False)
    log.info(
        "Feature info ~ Beta(%s,%s): min=%.4f  p50=%.4f  mean=%.4f  max=%.4f",
        cfg["dgp"]["info_dist"]["a"],
        cfg["dgp"]["info_dist"]["b"],
        info_df["info"].min(),
        info_df["info"].median(),
    ...
    # ── Histogram of info levels ─────────────────────────────────────────
    fig, ax = plt.subplots(figsize=(7, 4))
    ax.hist(info_df["info"], bins=30, color="steelblue", edgecolor="white", alpha=0.85)
    ax.axvline(info_df["info"].mean(), color="crimson", linewidth=2, linestyle="--", label=f"mean = {info_df['info'].mean():.4f}")
    ax.set_xlabel("Per‑feature information (separation)")
    ax.set_ylabel("Count")
    ax.set_title("Feature information distribution — Beta(1, 9)")
    ax.legend(frameon=False)
    ax.spines[["top", "right"]].set_visible(False)
    fig.tight_layout()
    fig.savefig(OUT_DIR / "fig_info_hist.png", dpi=130)
    plt.close(fig)
    log.saved("fig_info_hist.png")

    log.info(
        "DGP: %d features ~ Beta(%.1f,%.1f), p_pos=%.2f",
        len(features),
        cfg["dgp"]["info_dist"]["a"],
        cfg["dgp"]["info_dist"]["b"],
        cfg["dgp"]["p_pos"],
    ...
    for i, n_train in enumerate(sweep_values):
        log.section("n_train = %d (%d/%d)", n_train, i + 1, len(sweep_values))

        train_seed = seeds["train"] + i * 7
        df_train = dgp.sample(n_train, train_seed)
        y_train = df_train["y"].to_numpy()
        log.info(
            "train: n=%d  pos_rate=%.3f",
        ...
        for name, factory in factories.items():
            try:
                ...
                if cap is not None:
                    ...
                    log.ok("%-12s  cap=%.0f", name, cap)
                else:
                    log.ok("%-12s  cap=NA", name)
            except Exception as exc:  # noqa: BLE001
                log.warn("%-12s  FIT FAILED: %s", name, exc)
                fitted[name] = None
...
def plot_learning_curves(scores: pd.DataFrame, cfg: dict, metrics) -> None:
    ...
    for m in metrics:
        ...
        ax.set_title(f"{label} vs training size — 100 features ~ Beta(1,9) info")
...
def plot_gap_curves(scores: pd.DataFrame, cfg: dict) -> None:
    ...
    if baseline not in wide.columns:
        log.warn("gap baseline %s missing; skipping gap plot", baseline)
        return
...
def plot_capacity(cap_df: pd.DataFrame, cfg: dict) -> None:
    ...
    plt.close(fig)
    log.saved("fig_capacity_vs_n.png")
...
def write_report(
    ...
    lines: list[str] = []
    lines.append("# Weak Features (Beta‑distributed info) — Report\n")
    lines.append(
        "Research question: with 100 features whose information levels are "
        f"drawn from Beta({cfg['dgp']['info_dist']['a']}, "
        f"{cfg['dgp']['info_dist']['b']}) "
        f"(mean ≈ {info_df['info'].mean():.3f}, "
        f"p50 ≈ {info_df['info'].median():.3f}, "
        f"max ≈ {info_df['info'].max():.3f}, "
    ...
    lines.append("## Feature information summary\n")
    lines.append(
        f"| stat | value |\n"
        f"|---|---|\n"
        f"| count | {len(info_df)} |\n"
        f"| mean | {info_df['info'].mean():.4f} |\n"
        f"| median | {info_df['info'].median():.4f} |\n"
        f"| min | {info_df['info'].min():.4f} |\n"
        f"| max | {info_df['info'].max():.4f} |\n"
        f"| std | {info_df['info'].std():.4f} |\n"
    )
    lines.append(f"![info histogram](fig_info_hist.png)\n")
    ...
    lines.append("- `feature_info.csv` — per‑feature info_j ~ "
                 f"Beta({cfg['dgp']['info_dist']['a']}, "
                 f"{cfg['dgp']['info_dist']['b']})")
    lines.append("- `fig_info_hist.png` — histogram of sampled info levels")
    ...
    lines.append("- `fig_gap_vs_n.png` — AUC gap vs "
                 f"{setup_labels.get(baseline, baseline)}")
