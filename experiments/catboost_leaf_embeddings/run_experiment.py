...
def _plot_umap(
    ...
    # Hard labels
    colors_hard = np.where(y == 1, "#cc4444", "#4477bb")
    ax1.scatter(Z[:, 0], Z[:, 1], c=colors_hard, s=8, alpha=0.55, linewidths=0)
    ax1.set_title("Hard labels (binary y)", fontsize=10)
    ax1.set_xlabel("UMAP-1"); ax1.set_ylabel("UMAP-2")
    from matplotlib.patches import Patch
    ax1.legend(
        handles=[Patch(color="#cc4444", label="pos"), Patch(color="#4477bb", label="neg")],
        fontsize=8,
    )

    # Soft labels
    sc = ax2.scatter(Z[:, 0], Z[:, 1], c=y_soft, cmap="coolwarm", s=8, alpha=0.55,
